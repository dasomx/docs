...
1. **Get the HTTP streaming endpoint**:
   ...
   - Copy the generated HTTP streaming endpoint URL (Only need to copy the URL, not the whole command)

2. **Using a custom local server**:
...
<Warning>
  Use claude 3.5 sonnet model for best results. Other models may not work as
  expected.
</Warning>
...
1. **Connection Problems**:
   ...
   - Verify the HTTP streaming endpoint URL
