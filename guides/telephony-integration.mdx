...
- Phone number from a supported provider with voice capabilities
...
2. **Choose Your Provider**: Select either **Twilio** or **Custom SIP** as your telephony provider
3. **Enter Phone Number Details**: Fill in your phone number and provide a descriptive label
...
- **Phone Number**: Your phone number in E.164 format
- **Trunk Name**: Optional descriptive name for your SIP trunk

<Info>
  **Important**: Configure your SIP provider to route calls to: ```
  sip:p0joaqk9k11.sip.livekit.cloud ``` This is the OKChat origination URI for
  incoming calls.
</Info>
...
#### Custom SIP Outbound Setup

1. **Configure Outbound SIP**: Click "Configure Outbound SIP" for custom SIP providers
2. **SIP Configuration**: Enter your SIP provider details:
   - **Address**: Hostname or IP for SIP INVITE
   - **Transport Type**: TLS, TCP, or UDP
   - **Media Encryption**: Allowed, Required, or Disabled
   - **Authentication**: Optional username/password for digest auth
3. **Custom Headers**: Add any required SIP headers
...
- **Provider Badge**: Displays the telephony provider (Twilio, Custom SIP)
...
- **Setup Outbound**: Configure outbound calling (Twilio) or SIP settings (Custom)
- **Configure Outbound SIP**: Advanced SIP configuration for custom providers
...
- Number of connected phone numbers per provider
- Active vs total numbers
- Inbound configuration status
- Outbound configuration status
- Overall setup progress for each provider
...
- ✅ SIP routing is properly set up
...
- ✅ No firewall blocking SIP traffic
...
- Contact your SIP provider directly
- Verify SIP trunk configuration
- Check provider-specific documentation
- Ensure proper routing to OKChat endpoints
